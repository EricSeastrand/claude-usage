...
def find_biggest_jump_sessions(claude_dir, top_n=4):
...
def analyze_session_trajectory(claude_dir, session_prefix):
...
def main():
    parser = argparse.ArgumentParser(description="Analyze context jumps in sessions")
    parser.add_argument("--path", type=str, help="Path to Claude projects directory (default: ~/.claude/projects)")
    parser.add_argument("sessions", nargs="*", help="Session ID prefixes to analyze (auto-detects if omitted)")
    ...
    if args.sessions:
        ...
        print(f"Auto-detecting sessions with biggest context jumps in {claude_dir}...")
        targets = find_biggest_jump_sessions(claude_dir)
        print(f"Found: {', '.join(targets)}")

    for sid in targets:
        analyze_session_trajectory(claude_dir, sid)
